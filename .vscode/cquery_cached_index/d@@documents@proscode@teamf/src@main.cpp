   ...
   for(int i = 0; i < 10; i++){
	...
	while(true){
		...
		if(rightUp.isPressed() && rightDown.isPressed()){
			...
			if(FourBar.getPosition()>-100)
			while(LeftIntake.getPosition()>-300){
				intake(-200);
	 ...
	 if(btnA.isPressed()){							//ENTER DROPPER MODE
		 ...
		 }else if(Dropper.getPosition()>1550){
			 Dropper.moveVelocity(-200);
		 }else if(rightUp.isPressed()){					//Dropper up
